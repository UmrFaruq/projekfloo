...
// 🔥 IMPORT HALAMAN SETTING YANG BARU KITA BUAT 🔥
...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _fetchDashboardData() async {
    try {
      ...
      if (mounted) {
        setState(() {
          txtOmzet = "Rp ${NumberFormat('#,###', 'id_ID').format(omzet.toInt())}";
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      // 🔥 TOMBOL SETTING YANG SUDAH DI-UPDATE 🔥
                      IconButton(
                        icon: const Icon(
                          Icons.settings_outlined, // Iconnya saya ganti jadi roda gigi biar elegan
                          color: AppColors.primary,
                        ), 
                        tooltip: "Pengaturan POS",
                        onPressed: () {
                          // Arahkan ke Halaman AdminSettingsScreen
                          Navigator.push(
                            context,
                    ...
                    children: [
                      PopupMenuButton<String>(
                        ...
                        itemBuilder: (context) => [
                          const PopupMenuItem(
                            value: "Hari Ini",
                            child: Text("Hari Ini", style: TextStyle(color: Colors.black87)),
                          ),
                          const PopupMenuItem(
                            value: "Minggu Ini",
                            child: Text("Minggu Ini", style: TextStyle(color: Colors.black87)),
                          ),
                          const PopupMenuItem(
                            value: "Bulan Ini",
                            child: Text("Bulan Ini", style: TextStyle(color: Colors.black87)),
                          ),
                        ],
                        child: Container(
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          children: [
                            _buildMainStatCard(
                              "Omzet $filterWaktu", txtOmzet, "+12% tren naik", Icons.payments, AppColors.primary,
                            ),
                            const SizedBox(height: 12),
                            _buildMainStatCard(
                              "Perlu Restock", "$totalProdukMenipis Produk", "Stok menipis", Icons.inventory_2, AppColors.error,
                            ),
                          ],
                        ),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          children: [
                            _buildSmallStatCard(
                              "Transaksi", txtTransaksi, "Sukses diproses", Icons.receipt_long, AppColors.primary,
                            ),
                            const SizedBox(height: 12),
                            _buildSmallStatCard(
                              "Kasir Aktif", "$jumlahKasirAktif Kasir", "Terdaftar di sistem", Icons.people, AppColors.primary,
                            ),
                          ],
                        ),
                      ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(title, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, height: 1.2, color: Colors.black87), textAlign: TextAlign.center),
          ],
        ),
      ),
    );
