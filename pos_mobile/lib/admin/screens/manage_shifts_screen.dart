...
class _ManageShiftsScreenState extends State<ManageShiftsScreen> {
  ...
  Future<void> _fetchCashiers() async {
    try {
      ...
      cashierList = List<Map<String, dynamic>>.from(response);
    } catch (e) {
      debugPrint("ERROR FETCH CASHIER: $e");
    }
  }
  ...
  Future<void> _saveShift({String? shiftId}) async {
    try {
      final cashier = cashierList.firstWhere(
        (e) => e['id'] == selectedCashierId,
      );
      ...
      if (shiftId == null) {
        ...
        await AuditService.logActivity(
          action: "UPDATE SHIFT",
          detail: "Mengupdate shift ${_shiftNameController.text}",
          type: "update",
          userId: SessionService.userId,
        );
      }
      ...
      debugPrint("ERROR SAVE SHIFT: $e");

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Gagal simpan shift: $e"),
  ...
  void _shareSchedule() {
    if (shiftData.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Belum ada jadwal untuk dibagikan",
  ...
  void _showEditShiftSheet(int index, Map<String, dynamic> existingShift) {
    ...
    selectedCashierId = existingShift['user_id'];
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              try {
                ...
                await AuditService.logActivity(
                  action: "DELETE SHIFT",
                  detail: "Menghapus shift ${shift['nama_shift']}",
                  type: "delete",
                  userId: SessionService.userId,
                );
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              isEdit ? "Edit Jadwal Shift" : "Buat Jadwal Shift",
            ...
            DropdownButtonFormField<String>(
              ...
              items: cashierList.map((cashier) {
                return DropdownMenuItem<String>(
                  value: cashier['id'],
                  child: Text(cashier['name'] ?? cashier['username']),
                );
              }).toList(),
              onChanged: (value) {
                setState(() {
                  ...
                  child: Container(
                    ...
                    child: const Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 8),
                        Text(
                          "Pantau, atur, dan bagikan jadwal kerja kasir Anda.",
