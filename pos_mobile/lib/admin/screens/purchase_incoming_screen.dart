    ...
    try {
      ...
      for (var row in response) {
        ...
        if (row['tr_purchase'] != null &&
          ...
          supplierName =
              row['tr_purchase']['ms_supplier']['supplier_name'] ??
              supplierName;
          phoneNum =
              row['tr_purchase']['ms_supplier']['telephone_number']
                  ?.toString() ??
              "-";
        }
                    ...
                    DropdownButtonFormField<dynamic>(
                      ...
                      onChanged: (value) {
                        setModalState(() {
                          ...
                          final selectedProduct = dbProducts.firstWhere(
                            (p) => p['id'] == value,
                          );
  ...
  Future<void> _saveIncomingProduct() async {
    try {
      ...
      if (productName.isEmpty || qty <= 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Produk dan qty wajib diisi"),
            backgroundColor: AppColors.error,
          ),
        );
        return;
      ...
      if (supplierResponse == null) {
        final newSupplier = await supabase
            .from('ms_supplier')
            .insert({
              'supplier_name': supplierName.isEmpty
                  ? 'Supplier Umum'
                  : supplierName,
      ...
      final purchase = await supabase
          .from('tr_purchase')
          .insert({'supplier_id': supplierId, 'user_id': SessionService.userId})
          .select()
          .single();
      ...
      final currentQty = int.tryParse(productResponse['qty'].toString()) ?? 0;

      final newQty = currentQty + qty;

      await supabase
          .from('ms_product')
          .update({'qty': newQty, 'updated_by': SessionService.userId})
      ...
      await supabase.from('tr_stock').insert({
        ...
        'qty': qty,
        'tipe': 'in',
        'information': 'Purchase barang masuk $productName',
        'description': 'Penambahan stok dari purchase incoming',
        'type': 'purchase',
      });
      ...
      await AuditService.logActivity(
        ...
        detail: 'Menambahkan barang masuk $productName sebanyak $qty',
        type: 'create',
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Gagal simpan barang masuk: $e"),
          backgroundColor: AppColors.error,
        ),
      );
    }
              ...
              children: [
                  ...
                  child: Container(
                    ...
                    child: const Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 4),
                        Text(
                          "Klik item untuk melihat detail & kontak supplier",
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: TextField(
                      ...
                      onChanged: (value) =>
                          _runFilter(value), // Fitur filter jalan tiap diketik
                      style: const TextStyle(
                        color: Colors.black87,
                      ), // Teks search hitam
                      decoration: InputDecoration(
                ...
                Expanded(
                  child: Container(
                    ...
                    child:
                        filteredHistory
                            .isEmpty // Pakai list hasil filter pencarian
                        ? Center(
                            child: Column(
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return GestureDetector(
                                onTap: () =>
                                    _showDetailPopup(item), // MUNCULIN POPUP
                                child: Container(
