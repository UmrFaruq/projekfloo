...
class _ManageStockScreenState extends State<ManageStockScreen> {
  ...
  Future<void> _updateStock(String id, int currentTokoQty, int currentGudangQty, int pindahQty) async {
    try {
      ...
      await supabase.from('tr_stock').insert({
        ...
        'qty': pindahQty,
        'tipe': 'in',
        'information': 'Pindah stok Gudang ke Toko: ${product['name_product']} sebanyak $pindahQty',
      ...
      await AuditService.logActivity(
        action: 'MOVE STOCK',
        detail: 'Memindahkan stok ${product['name_product']} sebanyak $pindahQty ke etalase Toko',
        type: 'update',
      );
  ...
  void _showSnackBar(String m, {bool isError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(m), backgroundColor: isError ? AppColors.error : AppColors.primary), 
    );
  }
  ...
  void _showAddStockDialog(Map<String, dynamic> product) {
    ...
    final int currentGudangQty = _safeInt(product['qty_gudang']); // Stok Gudang Asli
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text("Pindah Stok ke Toko", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.black87)), 
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("Produk: ${product['name_product']}", style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.black87)), 
            ...
            Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.warehouse, color: AppColors.primary, size: 18),
                  const SizedBox(width: 8),
                  Text("Sisa di Gudang: $currentGudangQty $unit", style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary)),
                ],
              ),
            ),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              final int pindahQty = int.tryParse(qtyController.text) ?? 0;
              
              // 🔥 VALIDASI: GAK BOLEH LEBIH DARI STOK GUDANG 🔥
              if (pindahQty > currentGudangQty) {
                _showSnackBar("Gagal: Stok di Gudang tidak mencukupi!", isError: true);
                return;
              }

              if (pindahQty > 0) {
                Navigator.pop(ctx);
                _updateStock(product['id'].toString(), currentTokoQty, currentGudangQty, pindahQty);
              }
            },
            child: const Text("Pindah Stok", style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: const SizedBox(width: 260, child: AdminDrawer(activeMenu: "Manage Stock")),
      appBar: AppBar(
                  ...
                  child: filteredProducts.isEmpty
                      ...
                      : GridView.builder(
                          ...
                          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                            crossAxisCount: 2, crossAxisSpacing: 16, mainAxisSpacing: 16, childAspectRatio: 0.65, // <-- Diperpanjang dikit biar teks gudang muat
                          ),
                          itemCount: filteredProducts.length,
                          itemBuilder: (context, index) {
                            final p = filteredProducts[index];
                            final int qtyToko = _safeInt(p['qty']);
                            final int qtyGudang = _safeInt(p['qty_gudang']); // Ambil stok gudang
                            ...
                            return Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: isLowStock ? Border.all(color: AppColors.error.withOpacity(0.6), width: 1.5) : null, 
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      const Icon(Icons.warehouse, size: 10, color: AppColors.textGrey),
                                      const SizedBox(width: 4),
                                      Text("Gudang: $qtyGudang", style: const TextStyle(fontSize: 10, color: AppColors.textGrey)),
                                    ],
                                  ),
                                  const SizedBox(height: 4),
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          Text(
                                            ...
                                            style: TextStyle(fontSize: 9, color: isLowStock ? AppColors.error : AppColors.primary, fontWeight: FontWeight.bold),
                                          ), 
                                          Text(
                                            "$qtyToko $unit",
                                            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: isLowStock ? AppColors.error : AppColors.primary),
                                          ), 
                                        ],
                                      ),
                                      InkWell(
