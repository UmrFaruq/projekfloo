...
import 'package:pdf/widgets.dart' as pw; // <-- IMPORT WIDGET PDF
import 'package:printing/printing.dart'; // <-- IMPORT MESIN PRINT
...
class _ReceiptScreenState extends State<ReceiptScreen> {
  ...
  void initState() {
    ...
    saveOrder(); // Simpan ke memori lokal buat riwayat kasir
  }
  ...
  int getSubtotal() {
    int subtotal = 0;
    for (var item in cart) subtotal += item.price * item.qty;
    return subtotal;
  }
  ...
  Map<String, Map<String, int>> getMergedItems() {
    ...
    for (var item in cart) {
      if (items.containsKey(item.name)) items[item.name]!['qty'] = items[item.name]!['qty']! + item.qty;
      else items[item.name] = {'qty': item.qty, 'price': item.price};
    }
    return items;
  }
  ...
  void saveOrder() {
    ...
    final order = Order(
      id: transactionId, customer: widget.customer, paymentMethod: widget.paymentMethod,
      date: transactionDate, items: orderItems, subtotal: getSubtotal(), tax: getTax(), total: getTotal(),
    );
    allOrders.value = [...allOrders.value, order];
    shiftOrders.value = [...shiftOrders.value, order];
  }

  // 🔥 FUNGSI SAKTI UNTUK MENCETAK KE PRINTER / BIKIN PDF 🔥
  Future<void> _printReceipt() async {
    ...
    pdf.addPage(
      pw.Page(
        pageFormat: PdfPageFormat.roll80, // Ukuran kertas struk kasir (Printer Thermal 80mm)
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text("Tax (12%)", style: const pw.TextStyle(fontSize: 10)), pw.Text(formatRupiah(getTax()), style: const pw.TextStyle(fontSize: 10))]),
              
              pw.SizedBox(height: 4),
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                pw.Text("TOTAL", style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)), 
                pw.Text(formatRupiah(getTotal()), style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold))
              ]),
              ...
              if (widget.paymentMethod == "cash" && widget.amountPaid != null) ...[
                ...
                pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text("Kembalian", style: const pw.TextStyle(fontSize: 10)), pw.Text(formatRupiah(widget.amountPaid! - getTotal()), style: const pw.TextStyle(fontSize: 10))]),
              ],
    ...
    // MENGELUARKAN POP-UP PRINT DARI BROWSER/WINDOWS
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => pdf.save(),
      name: 'Struk_${transactionId}', // Nama file PDF defaultnya
    );
  }
                ...
                child: Container(
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        // ... BAGIAN UI STRUK SAMA SEPERTI SEBELUMNYA ...
                        ...
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Tax (12%)", style: TextStyle(fontSize: 13, color: Colors.black87)), Text(formatRupiah(getTax()), style: const TextStyle(fontSize: 13, color: Colors.black87))]),
                        const SizedBox(height: 10),
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("TOTAL", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.black87)), Text(formatRupiah(getTotal()), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.black87))]),
                        ...
                        if (widget.paymentMethod == "cash" && widget.amountPaid != null) ...[
                          ...
                          const SizedBox(height: 4),
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Kembalian", style: TextStyle(fontSize: 13, color: Colors.black87)), Text(formatRupiah(widget.amountPaid! - getTotal()), style: const TextStyle(fontSize: 13, color: Colors.black87))]),
                        ],
                ...
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      ...
                      onPressed: _printReceipt, // 🔥 MANGGIL FUNGSI PDF PRINTING 🔥
